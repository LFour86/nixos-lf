...
| `gost-pac`（uid 987） | `127.0.0.1:33332`（HTTP）、`127.0.0.1:33333`（透明重定向） | 本机中转；转发到 mihomo，核心不可用时直接拒绝连接 |
...
`gost-pac` 是 fail-closed 的：它只把流量转发到 `127.0.0.1:7897`，因此核心不可用期间 `:33332`/`:33333` 的连接会被直接拒绝，而不是回退为直接出网——真实地址不会被当作隐式回退。DNS 仍会切换：mihomo 不可用期间 `dns-pac` 把 dnsmasq 指向加密解析器。
...
* 断网保护会丢弃非 root 进程的直接出站流量，回环、局域网地址、DNS/NTP 与组播除外。uid 987（`gost-pac`）仍被豁免于断网保护与 mihomo 的 TUN；在中继改为 fail-closed 之后，该豁免已是遗留项而非出口路径。
* 探测只认属于 `clash-verge.service` 的监听者：判据读取监听者的 cgroup，本机进程无法伪造，因此仅在 `127.0.0.1:7897` 上监听并不能把流量引过去。
